...
PopupWindow {
    ...
    readonly property color tabColor: {
        if (activeTab === "outputs") return "#2196F3"
        if (activeTab === "inputs") return "#9C27B0"
        return "#4CAF50"
    }
    ...
    QtObject {
        ...
        function cycleProfile(cardName) {
            Quickshell.execDetached(["bash", "-c", 
                "CARD='" + cardName + "'; " +
                "CURRENT=$(pactl list cards | awk '/Name:.*" + cardName + "/{found=1} found && /Active Profile:/{print $NF; exit}'); " +
                "FOUND=0; " +
                "for PROFILE in a2dp-sink a2dp-sink-sbc_xq headset-head-unit headset-head-unit-cvsd; do " +
                "  if [ $FOUND -eq 1 ]; then pactl set-card-profile $CARD $PROFILE 2>/dev/null && break; fi; " +
                "  if [ \"$PROFILE\" = \"$CURRENT\" ]; then FOUND=1; fi; " +
                "done; " +
                "if [ $FOUND -eq 1 ]; then pactl set-card-profile $CARD a2dp-sink 2>/dev/null; fi"
            ])
            Qt.callLater(getAudioState, 500)
        }
        ...
        function saveVolumes() {
            if (outputsModel.count === 0) return
            
            let data = { outputs: [], inputs: [], apps: [] }
            
            for (let i = 0; i < outputsModel.count; i++) {
                let d = outputsModel.get(i)
                data.outputs.push({ name: d.name || "", volume: d.volume || 0, mute: d.mute || false })
            }
            
            for (let i = 0; i < inputsModel.count; i++) {
                let d = inputsModel.get(i)
                data.inputs.push({ name: d.name || "", volume: d.volume || 0, mute: d.mute || false })
            }
            
            for (let i = 0; i < appsModel.count; i++) {
                let d = appsModel.get(i)
                data.apps.push({ description: d.description || "", volume: d.volume || 0, mute: d.mute || false })
            }
        ...
        function restoreVolumes() {
            if (outputsModel.count === 0 && inputsModel.count === 0) return
            ...
            if (fingerprint === lastRestoredDevices) return
    ...
    Component.onCompleted: {
        ...
        Qt.callLater(function() { audioManager.restoreVolumes() }, 2000)
    }
    ...
    function syncModel(listModel, dataArray) {
        dataArray.sort(function(a, b) {
            ...
            function isExternal(desc, name) {
                if (desc.indexOf("headset") !== -1) return true
                if (desc.indexOf("headphone") !== -1) return true
                if (desc.indexOf("buds") !== -1) return true
                if (desc.indexOf("pods") !== -1) return true
                if (desc.indexOf("bluetooth") !== -1) return true
                if (desc.indexOf("usb") !== -1) return true
                if (name.indexOf("bluez") !== -1) return true
                return false
            }
        ...
        for (let i = 0; i < dataArray.length; i++) {
            ...
            if (foundIdx === -1) {
                ...
                for (let key in obj) {
                    if (key === "volume" && draggingNodes[obj.id]) continue
    ...
    function updateHero() {
        ...
        for (let i = 0; i < targetModel.count; i++) {
            ...
            if (d.is_default) {
                ...
                if (d.name.indexOf("bluez") !== -1) getAudioProfile(d.name)
                return
            }
        }
    ...
    function getProfileLabel(profile) {
        ...
        if (profile.indexOf("a2dp-sink-sbc_xq") !== -1) return "A2DP (SBC-XQ)"
        if (profile.indexOf("a2dp-sink") !== -1) return "A2DP (SBC)"
        if (profile.indexOf("headset-head-unit-cvsd") !== -1) return "HFP (CVSD)"
        if (profile.indexOf("headset-head-unit") !== -1) return "HFP (mSBC)"
        if (profile.indexOf("headset") !== -1) return "Headset"
        if (profile.indexOf("a2dp") !== -1) return "A2DP"
        if (profile.indexOf("off") !== -1) return "Off"
        return profile
    }

    function getAudioProfile(desc) {
        let cardName = desc
        if (cardName.indexOf("bluez_output.") !== -1) cardName = cardName.replace("bluez_output.", "bluez_card.")
        else if (cardName.indexOf("bluez_input.") !== -1) cardName = cardName.replace("bluez_input.", "bluez_card.")
    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                transform: Translate { y: 15 * (1 - soundPopup.introHero) }
                    ...
                    Rectangle {
                        ...
                        Canvas {
                            ...
                            onPaint: {
                                ...
                                var r = width / 2
                                var fillY = height * (1.0 - activeVol / (volumeBoost ? 150 : 100))
                                ctx.save()
                                ctx.beginPath()
                                ...
                                if (activeVol < ((volumeBoost ? 150 : 100)-1)) {
                                    ...
                                    ctx.bezierCurveTo(
                                        width * 0.33,
                                        fillY + Math.sin(wavePhase) * amp,
                                        width * 0.66,
                                        fillY + Math.cos(wavePhase + Math.PI) * amp,
                                        width,
                                        fillY
                                    )
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                onPositionChanged: (mouse) => {
                                    if (pressed) updateMaster(mouse.x)
                                }
                                onReleased: {
                                    syncDelay.restart()
                                    audioManager.getAudioState()
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                anchors.fill: parent
                                cursorShape: Qt.PointingHandCursor
                                onPressed: (mouse) => updateBalance(mouse.x)
                                onPositionChanged: (mouse) => {
                                    if (pressed) updateBalance(mouse.x)
                                }
            ...
            Rectangle {
                ...
                transform: Translate { y: 15 * (1 - soundPopup.introTabs) }
                ...
                RowLayout {
                    ...
                    TabButton { text: "󰓃 Outputs"; tabId: "outputs" }
                    TabButton { text: "󰍬 Inputs"; tabId: "inputs" }
                    TabButton { text: "󰎆 Streams"; tabId: "apps" }
                }
            }
            ...
            Rectangle {
                ...
                transform: Translate { y: 15 * (1 - soundPopup.introList) }
                        ...
                        RowLayout {
                            ...
                            Text {
                                text: {
                                    ...
                                    if (desc.indexOf("headset") !== -1 || desc.indexOf("headphone") !== -1) return "󰋎"
                                    return "󰓃"
                                }
                                ...
                                Rectangle {
                                    ...
                                    MouseArea {
                                        ...
                                        onClicked: {
                                            ...
                                            Qt.callLater(function() { audioManager.getAudioState() }, 300)
                                        }
                                    }
                                }
                            }
