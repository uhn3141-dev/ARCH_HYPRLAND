...
PopupWindow {
    ...
    onVisibleChanged: {
        if (visible) {
            introOrbs = 0; introTabs = 0; introContent = 0
            entryAnim.restart()
    ...
    readonly property color tabColor: {
        if (activeTab === "cpu") return cpuColor
        if (activeTab === "gpu") return gpuColor
        if (activeTab === "memory") return memColor
        if (activeTab === "disk") return diskColor
        return "#FF9800"
    }
    ...
    ParallelAnimation {
        id: entryAnim
        NumberAnimation { target: sysPopup; property: "introOrbs"; from: 0; to: 1; duration: 400; easing.type: Easing.OutBack }
        SequentialAnimation {
            PauseAnimation { duration: 100 }
            NumberAnimation { target: sysPopup; property: "introTabs"; from: 0; to: 1; duration: 400; easing.type: Easing.OutBack }
        }
        SequentialAnimation {
            PauseAnimation { duration: 200 }
            NumberAnimation { target: sysPopup; property: "introContent"; from: 0; to: 1; duration: 450; easing.type: Easing.OutBack }
        }
    }
    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                transform: Translate { y: 15 * (1 - sysPopup.introOrbs) }
            ...
            Rectangle {
                ...
                color: theme.colSecondary; radius: 10
                opacity: sysPopup.introTabs
                transform: Translate { y: 15 * (1 - sysPopup.introTabs) }
                RowLayout {
                    anchors.fill: parent; anchors.margins: 3; spacing: 3
                    SysTab { text: "CPU"; tabId: "cpu" }
                    SysTab { text: "GPU"; tabId: "gpu" }
                    SysTab { text: "Memory"; tabId: "memory" }
                    SysTab { text: "Disk"; tabId: "disk" }
                    SysTab { text: "Apps"; tabId: "apps" }
                }
            }
            ...
            Rectangle {
                ...
                transform: Translate { y: 15 * (1 - sysPopup.introContent) }
                                        ...
                                        Item {
                                            ...
                                            RowLayout {
                                                ...
                                                Column {
                                                    Layout.fillWidth: true
                                                    spacing: 6
                                                    InfoRow { label: "Usage";       value: (cpuData.usage || 0) + "%" }
                                                    InfoRow { label: "Cores";       value: cpuData.cores ? cpuData.cores : "-" }
                                                    InfoRow { label: "Frequency";   value: cpuData.freq ? cpuData.freq + " GHz" : "-" }
                                                    InfoRow { label: "Temperature"; value: cpuData.temp ? cpuData.temp + "°C" : "-" }
                                                ...
                                                Column {
                                                    Layout.fillWidth: true
                                                    spacing: 6
                                                    InfoRow { label: "L2 Cache";    value: cpuData.l2 || "-" }
                                                    InfoRow { label: "L3 Cache";    value: cpuData.l3 || "-" }
                                                    InfoRow { label: "Governor";    value: cpuData.governor || "-" }
                                                    InfoRow { label: "Processes";   value: cpuData.processes || "-" }
                                                }
                                            }
                                        }
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        Column {
                            ...
                            InfoRow { label: "Usage";       value: (gpuData.usage || 0) + "%" }
                            InfoRow { label: "Frequency";   value: gpuData.freq ? gpuData.freq + " MHz" : "-" }
                            InfoRow { label: "Temperature"; value: gpuData.temp ? gpuData.temp + "°C" : "-" }
                            InfoRow { label: "VRAM Used";   value: gpuData.vram ? gpuData.vram + "M" : "-" }
                        }
                        Column {
                            Layout.fillWidth: true; spacing: 6
                            InfoRow { label: "VRAM Total";  value: gpuData.vram_total ? gpuData.vram_total + "M" : "-" }
                            InfoRow { label: "Driver";      value: gpuData.driver || "-" }
                            InfoRow { label: "PCIe";        value: gpuData.pcie || "-" }
                        }
                    }
                }
                Rectangle { width: parent.width; height: 10; color: "transparent" }
...
Item {
            ...
            Column {
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        Column {
                            Layout.fillWidth: true; spacing: 6
                            InfoRow { label: "Usage";      value: (memData.usage || 0) + "%" }
                            InfoRow { label: "Total";      value: memData.total || "-" }
                            InfoRow { label: "Used";       value: memData.used || "-" }
                            InfoRow { label: "Available";  value: memData.available || "-" }
                            InfoRow { label: "Cached";     value: memData.cached || "-" }
                        }
                        Column {
                            Layout.fillWidth: true; spacing: 6
                            InfoRow { label: "Buffers";    value: memData.buffers || "-" }
                            InfoRow { label: "Swap Total"; value: memData.swap_total || "-" }
                            InfoRow { label: "Swap Used";  value: memData.swap_used || "-" }
                            InfoRow { label: "Swap %";     value: memData.swap_pct ? memData.swap_pct + "%" : "0%" }
                        }
                    }
                }
                Rectangle { width: parent.width; height: 10; color: "transparent" }
    ...
    component UsageOrb: Rectangle {
        ...
        Canvas {
            ...
            Connections {
                target: parent
                function onAnimUsageChanged() { orbCanvas.requestPaint() }
            }
        }
