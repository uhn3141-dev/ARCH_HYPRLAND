...
PopupWindow {
    ...
    property real introRow1: 0
    // Uptime + Power Mode
    property real introRow2: 0
    // Tools
    property real introRow3: 0
    ...
    component AnimatedPowerButtons: Rectangle {
        ...
        property real weight: 1 // Hold time multiplier
        ...
        Canvas {
            ...
            onPaint: {
                ...
                if (btn.fillLevel <= 0.001)
                    return ;

                var r = 25;
                var fillY = height * (1 - btn.fillLevel);
                ctx.save();
                ctx.beginPath();
                ctx.moveTo(r, 0);
            ...
            NumberAnimation on wavePhase {
                running: btn.fillLevel > 0 && btn.fillLevel < 1
        ...
        MouseArea {
            ...
            onReleased: {
                if (!btn.triggered && btn.fillLevel < 1) {
                    fillAnim.stop();
                    drainAnim.start();
                }
            }
    ...
    component ToolButton: Rectangle {
        ...
        Text {
            ...
            rotation: {
                if (animType === "spin" && active)
                    return 360;

                if (animType === "layout") {
                    if (activeColor === "#2196F3")
                        return 0;

                    if (activeColor === "#4CAF50")
                        return 120;
            ...
            scale: (animType === "scale" && active) ? 1.3 : 1
